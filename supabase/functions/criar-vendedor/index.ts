...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (codigo_vendedor && typeof codigo_vendedor !== 'string') {
      return new Response(
        JSON.stringify({ error: 'Código do vendedor deve ser uma string' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (telefone && typeof telefone !== 'string') {
      return new Response(
        JSON.stringify({ error: 'Telefone deve ser uma string' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (codigo_vendedor) {
      ...
      if (existingProfile) {
        return new Response(
          JSON.stringify({ error: 'Este código de vendedor já está em uso' }),
          { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
    }
